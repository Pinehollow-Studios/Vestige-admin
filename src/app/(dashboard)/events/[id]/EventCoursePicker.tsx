...
import { addEventCourse } from "../actions";
import type { CourseCatalogRow } from "../../curated/types";

/**
 * Course picker for an event's set — the same on-demand
 * `/api/courses/search` idiom as the curated-list picker, wired to
 * `addEventCourse`.
 */
export function EventCoursePicker({
  ...
  function add(courseId: string) {
    ...
    startTransition(async () => {
      const result = await addEventCourse(eventId, courseId);
      setPendingId(null);
      if (!result.ok) {
      ...
      <div className="max-h-80 overflow-y-auto rounded-md border">
        {loading && results.length === 0 ? (
          ...
          <ul className="divide-y text-sm">
            {results.map((row) => {
              const onSet = alreadyOnSet.has(row.course_id) || added.has(row.course_id);
              ...
              return (
                <li key={row.course_id} className="flex items-center justify-between gap-2 px-3 py-2">
                  ...
                  {onSet ? (
                    <span className="shrink-0 text-xs text-muted-foreground">On set</span>
                  ) : (
                    <Button
                      size="sm"
                      variant="outline"
