...
- **2026-06-27** — Course dataset import in the dashboard (`/courses/import`).
  ...
  in the separate `vestige-tool` app, committed to its `src/courses.js` — can be
  ...
  **apply is super_admin-only** (writes live course data via service-role, audited
  ...
  Contents:read on `Pinehollow-Studios/vestige-tool` (else a "not configured"
...
- **2026-08-18** — **Age derived from founding year; weights re-balanced**
  (age 0.20 / ranking 0.40 / setting 0.40 — ranking + setting lead). Age is
  the one axis the data can fill itself (1,793/1,794 courses have a year), so
  it's computed from an era-anchored curve (`AGE_ANCHORS` + `ageFromYear()` in
  `vestige-index/formula.ts`: 1766→100 … 2026→35) rather than hand-scored;
  linear was rejected because the distribution is bimodal (52% 1880-1939, 37%
  1970-1999, only 16 courses pre-1880) and the 35 floor stops a good modern
  course being capped by its age. Backfill SQL generated by evaluating the TS
  function itself so stored values can't drift from code; applied prod (1,790
  scored) + dev (1,773). Four bad rows fixed first — three mis-parsed OSM years
  nulled rather than guessed, plus Chichester's genuine gap; all four need a
  real year from Jack. Index currently spans 39-65 only because ranking +
  setting are still empty. Verified `tsc`/`eslint` + spot-checks. Long-form in
  `CHANGELOG.md`.
